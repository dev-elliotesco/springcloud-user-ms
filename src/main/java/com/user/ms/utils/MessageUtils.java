package com.user.ms.utils;

public class MessageUtils {

    public static final String USER_NOT_FOUND = "User not found with id: ";
    public static final String USER_DELETED_SUCCESSFULLY = "User deleted successfully";
    public static final String USER_ALREADY_EXISTS = "El usuario ya existe";
    public static final String ONLY_NUMBERS_ALLOWED = "Solo se permite numeros";
    public static final String DOCUMENT_SIZE_CONSTRAINT = "Document size must be between 2 and 10";
    public static final String TYPE_DOCUMENT_CONSTRAINT = "el dato debe ser CC o CE";
    public static final String NAME_SIZE_CONSTRAINT = "Name size must be between 3 and 8";
}
